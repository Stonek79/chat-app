...
5.  **[ ] Создать файл для Zod-схем аутентификации:**
    - **Действие:** Создать новый файл `src/types/auth/authSchemas.ts`.

6.  **[ ] Определить Zod-схемы для API аутентификации:**
    - **Действие:** В `src/types/auth/authSchemas.ts` определить и экспортировать:
        - `registerSchema` (для `POST /api/auth/register`)
        - `loginSchema` (для `POST /api/auth/login`)
        - `userResponseSchema` (для ответа `GET /api/auth/me`)
    - **TSDoc:** Добавить TSDoc-комментарии к каждой схеме.

7.  **[ ] Трансформировать TypeScript типы аутентификации:**
    - **Действие:** В файле `src/types/auth/authTypes.ts` удалить дублирующиеся типы и заменить их на `z.infer` из `authSchemas.ts`.

8.  **[ ] Централизовать валидацию API:**
    - **Действие:** В `src/app/api/auth/register/route.ts` и `src/app/api/auth/login/route.ts`:
        - Удалить локально определенные Zod-схемы.
        - Импортировать и использовать `registerSchema` и `loginSchema` из `src/types/auth/authSchemas.ts`.
...
- **Задачи:**
    1.  [ ] Установить зависимость: `npm install react-hot-toast`.
    2.  [ ] Добавить `<Toaster />` в корневой `src/app/layout.tsx`.
    3.  [ ] Модифицировать хуки, работающие с API (`useAuth`, `useChat`): добавить `error` в состояние, возвращать его и вызывать `toast.error()` в `catch`.
    4.  [ ] Обновить UI-компоненты (`LoginForm`) для отображения `error` из хуков.
...
- **[ ] Получение списка чатов (`GET /api/chats`):**
    - [ ] Реализовать API эндпоинт.
    - [ ] Аутентификация пользователя.
    - [ ] Логика получения списка чатов, где пользователь является участником.
    - [ ] Включить в запрос данные о последнем сообщении и участниках (`_count` для непрочитанных).
    - [ ] Отсортировать чаты по времени последнего сообщения.
    - [ ] Реализовать маппинг в `ClientChat[]`.
- **[ ] Получение истории сообщений (`GET /api/chats/[chatId]/messages`):**
    - [ ] Реализовать API эндпоинт.
    - [ ] Аутентификация и проверка, что пользователь является участником чата.
    - [ ] Реализовать пагинацию (cursor-based pagination по `createdAt` или `id`).
    - [ ] Включить в запрос данные об отправителе.
    - [ ] Реализовать маппинг в `ClientMessage[]`.
