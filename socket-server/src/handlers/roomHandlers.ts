...
async function handleJoinChat(socket: AppServerSocket, chatId: string) {
...
async function handleLeaveChat(socket: AppServerSocket, chatId: string) {
    const user = socket.data.user;
    if (!user) {
...
export const registerRoomHandlers = (socket: AppServerSocket) => {
